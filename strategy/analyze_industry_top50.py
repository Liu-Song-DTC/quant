...
def analyze_group(group_name, all_data, forward=20, top_n=50):
    """分析单个分组，输出Top N因子"""
    if len(all_data) < 150:
        return None
...
def main():
    print("="*70)
    print("全股票因子挖掘 - 每个行业输出Top50高IC因子")
    print("="*70)
    ...
    for industry, stocks in sorted_ind:
        if len(stocks) < 5:
            continue

        print(f"\n{'='*60}")
        print(f"行业: {industry} ({len(stocks)}只) - Top {top_n} 因子")
        print(f"{'='*60}")

        group_data = all_df[all_df['industry'] == industry].to_dict('records')
        result = analyze_group(industry, group_data, forward, top_n)
        if result:
            results[industry] = result
            print(f"  样本: {result['sample']}")
            print(f"\n  Top 30 因子 (按|IC|):")
            for i, (k, v) in enumerate(result['factors'][:30]):
                sig = "✓✓✓" if abs(v) >= 0.08 else "✓✓" if abs(v) >= 0.04 else "✓"
                direction = "↑" if v > 0 else "↓"
                print(f"    {i+1:2d}. {k:25s}: {v:+.4f} {direction} {sig}")

    # 全部数据分析
    print(f"\n{'='*60}")
    print(f"全部股票汇总 - Top {top_n} 因子")
    print(f"{'='*60}")
    result_all = analyze_group('全部', all_stock_data, forward, top_n)
    if result_all:
        results['全部'] = result_all
        print(f"  样本: {result_all['sample']}")
        print(f"\n  Top 30 因子 (按|IC|):")
        for i, (k, v) in enumerate(result_all['factors'][:30]):
            sig = "✓✓✓" if abs(v) >= 0.08 else "✓✓" if abs(v) >= 0.04 else "✓"
            direction = "↑" if v > 0 else "↓"
            print(f"    {i+1:2d}. {k:25s}: {v:+.4f} {direction} {sig}")
    ...
    for name, r in results.items():
        ...
        top5 = r['factors'][:5]
        for k, v in top5:
            direction = "正" if v > 0 else "反"
            print(f"  {k}: {v:+.4f} ({direction}向)")
    ...
    for name, r in results.items():
        output[name] = {
            'sample': r['sample'],
            'stock_count': len(industry_stocks.get(name, [])),
            'top_factors': [(k, float(v)) for k, v in r['factors']]
        }

    out_path = os.path.join(BASE_DIR, 'strategy/industry_top50_factors_ic.json')
    ...
    for name, r in results.items():
        if name == '全部':
            continue
        # 选取Top5因子，IC>4%的
        valid_factors = [(k, v) for k, v in r['factors'] if abs(v) >= 0.04][:5]
        if valid_factors:
            config[name] = {
                'factors': [k for k, v in valid_factors],
                'direction': {k: 'positive' if v > 0 else 'negative' for k, v in valid_factors}
            }

    config_path = os.path.join(BASE_DIR, 'strategy/industry_signal_config.json')
    with open(config_path, 'w', encoding='utf-8') as f:
        json.dump(config, f, ensure_ascii=False, indent=2)
    print(f"信号系统配置已保存到: {config_path}")
