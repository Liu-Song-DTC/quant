...
INDUSTRY_CHAINS: Dict[str, Dict[str, List[str]]] = {
    ...
    "AI人工智能": {
        ...
        "芯片与通信":  ["国产芯片", "半导体概念", "F5G概念", "PCB", "华为概念", "通信技术"],
    ...
    "数字经济": {
        ...
        "基础设施":   ["数据中心", "云计算", "算力概念", "F5G概念", "互联网服务"],
    ...
    # ═══ E-D1补充(2026-09-05): 回测告警中的高频缺失主导行业 ═══
    # 基线no_chain=74次(命中率52%), 这些行业成为主导时产业链聚焦退回不限行业。
    # 成员均取信号行业词汇表中的真实值(词汇表核对过); "天然气"无同族词,
    # 归入_NO_CHAIN_CONCEPTS免告警而非硬造链条。
    "CPO概念": {
        "光模块":     ["光通信模块", "F5G概念", "通信技术"],
        "算力配套":   ["算力概念", "数据中心", "云计算", "AI应用"],
        "器件支撑":   ["半导体概念", "国产芯片", "PCB"],
    },
    "固态电池": {
        "电池体系":   ["电池技术", "动力电池回收", "钠离子电池", "锂电池概念"],
        "材料":       ["新材料", "化工原料"],
        "应用":       ["新能源车", "储能概念", "新能源"],
    },
    "IGBT概念": {
        "功率器件":   ["半导体概念", "国产芯片"],
        "下游应用":   ["新能源车", "充电桩", "智能电网", "特高压"],
    },
    "MLCC": {
        "被动元件":   ["半导体概念", "国产芯片", "PCB", "LED概念"],
        "应用":       ["消费电子概念", "AI应用", "新能源车"],
    },
    "OLED": {
        "面板器件":   ["LED概念", "半导体概念", "国产芯片"],
        "应用":       ["消费电子概念", "AI眼镜", "智能穿戴"],
    },
    "华为汽车": {
        "整车":       ["新能源车", "小米汽车", "汽车整车"],
    ...
    "BC电池": {
        "电池片":     ["光伏概念", "HJT电池", "TOPCon电池"],
        "应用":       ["新能源", "储能概念", "电池技术"],
    },
    "PEEK材料概念": {
        "材料":       ["新材料", "化工原料"],
        "应用":       ["人形机器人", "机器人概念", "低空经济", "航天航空", "商业航天"],
    },
    "6G概念": {
        "通信":       ["F5G概念", "通信技术", "光通信模块"],
        "配套":       ["数据中心", "算力概念", "物联网", "PCB"],
    },
    "ERP概念": {
        "软件":       ["信创", "国产软件", "云计算", "大数据"],
        "应用":       ["AI应用", "东数西算"],
...
_NO_CHAIN_CONCEPTS: set = {
    # 宽基/风格/主题概念
    "天然气",  # E-D1(2026-09-05): 信号行业词汇表中无同族词(无石油/燃气/油服), 硬造链条无意义
    ...
    "养老金", "独角兽",
    "长期破净", "造纸印刷",
	    "智慧灯杆", "科创板做市股", "虚拟数字人", "ETC",
	    "家居用品", "交运设备", "装修装饰", "通用设备",
	    "装修建材", "轨交设备", "超清视频", "首发经济", "量子科技",
}
...
def _build_chain_index() -> Dict[str, set]:
    ...
    for chain_name, tiers in INDUSTRY_CHAINS.items():
        ...
        for tier_name, concepts in tiers.items():
            all_c.update(concepts)
        for c in all_c:
            if c not in idx:
                idx[c] = set()
            idx[c].update(all_c)
        # 链条名本身也入索引: E-D1新链条用真实行业名(CPO概念/固态电池等),
        # 主导行业=链条名时 get_chain_concepts 同样要返回整链(旧抽象类目链名
        # 从不作为候选行业出现, 此缺陷未暴露)
        idx.setdefault(chain_name, set()).update(all_c)
    return idx
