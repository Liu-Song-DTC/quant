...
### 17.12: 门控家族扩展探针 — 全类×sl分桶穷尽 (9/23 07:05)

sl4采纳后在**新生产信号集**上执行 #211: signal_validator重生成
validation_results.csv (6,580,994行, 06:37) → 流式join (bp×sl) 分桶
fwd10 → 入场归因 (portfolio_selections 574行→569入场事件×signals×fwd10)。

**(bp×sl) 信号层分桶** (1,183,057 buy行): 关键读数:

| 桶 | n | win% | mean fwd10 | 年分解 |
|---|---|---|---|---|
| bp1 sl2 (边际, 门=2) | 73,847 | 54.2 | **+1.49%** | 6/6年正 |
| bp7 sl4 (边际, 门=4) | 2,671 | 59.2 | **+1.91%** | 6/6年正 |
| bp8 sl2 (边际, 门=2) | 38,896 | 50.2 | **+0.80%** | 2022 −1.4/2026 +0.1 |
| bp5 sl2 (边际, 门=2) | 11,988 | 48.7 | +0.22% | 2022/2026 −0.6/−1.3 |
| bp2 sl-1/0/1 (无门) | 5,872/4,433/11,924 | 58.7/61.6/60.2 | **+2.41/+3.07/+2.77%** | 全正 |
| bp9 sl0 (无门) | 1,612 | 40.1 | **−1.50%** | 5/6年负 (2026 −5.1) |
| bp3 sl0 (b3关) | 1,063 | 41.9 | −1.23% | 年交替混合 |
| bp0 sl-1 (主路径) | 186,485 | 47.1 | −0.07% | 近零EV |

**裁决链**:
- 边际行全正 → bp1/bp5/bp7/bp8 门爬升臂全否决 (切正EV行方向错). bp7
  sl4 +1.91%佐证flat-top采纳 (sl5切掉的恰是好行, −100 NAV已含此成本).
- **E-K1sl否决**: bp2 sl0/sl1行 +3.07/+2.77% 全正 → bp2加sl门是反向切割,
  bp2类级加成(E-K1)保持现状. 探针省下40min臂.
- bp9 sl0信号层−1.50%但**入场归因n=1** (5.5年, +3.9%那笔); bp3 sl0
  同样n=1 (−21.6%那笔) → 组合层零行为 (低分永不被选) → 门臂关闭.
  b3_filter=disabled系ee58787(7/21配置优化)批量改动, 从未单独立bracket,
  但重新启用=零行为修复, 不值得3.5h臂.
- bp0 sl-1 186k行近零EV但E-N5家族(无结构bp0硬过滤/软罚加重)已否决,
  且入场n=19混合 → 关闭.

**#211终局: 门控家族空间穷尽.** 现有10类×sl门全部坐于正边际; 无门类
(bp2/bp3/bp9)或全正或零入场. 下一切集不存在. 探针产物:
/tmp/probe_bp_sl_buckets_20260923.txt, /tmp/probe_entry_attribution_20260923.txt.

### 17.13: E-ZG1中枢追高家族终局 — 软罚−0.20采纳, 新锚点1,001,332 (9/23 08:25)

**家族史**: 探针(chase行6/6年负, close≥zg fwd −2.38% vs <zg +0.49%, 部分
相关−0.162完全正交score) → E-ZG1硬拒臂(3-1 MDD败, 2026 −11.5pp位移级联)
→ 全score分位负(chase×score探针) → 细化bracket:

| 变体 | NAV | 收益 | Sharpe | MDD | 裁决 |
|---|---|---|---|---|---|
| 锚(sl4, 无过滤) | 853,076 | 241.23 | 1.3556 | 15.99 | ref |
| 硬拒(14,441行) | 1,443,416 | 477.37 | 1.8004 | 18.13 | 3-1 MDD败 |
| 软罚0.08 | 940,344 | 276.14 | 1.4528 | 16.25 | 3-1 MDD败 |
| 软罚0.15 | 1,011,479 | 304.59 | 1.5338 | 15.63 | 4-0 |
| **软罚0.20 (采纳)** | **1,001,332** | **300.53** | **1.5137** | **15.46** | **4-0** |
| 软罚0.25 | 1,014,348 | 305.74 | 1.5269 | 15.82 | 4-0 |
| 分层0.08/0.20 | 911,542 | 264.62 | 1.4285 | 16.41 | 3-1 MDD败 |

**裁决链**:
- 硬拒/深chase/豁免/分层全否决: 硬拒位移级联(546 vs 574入场, 2026路径毁灭);
  分层浅档0.08与平坦0.08同签名(MDD败) — 微桶梯度≠组合层最优.
- 平坦软罚4-0平台=[0.15, 0.25], 三4-0点NAV±1.3%/Sharpe±0.02/MDD±0.36pp
  = 度量层CI以下真flat-top → **采纳中心0.20**(最佳MDD 15.46), 非argmax.
- identity裸跑(默认值烤入)逐位复现, equity_curve byte-identical.
- 年分解: 5/6年收益+1.8~11.4pp, 2026 −14.5pp(位移残差), 2021-25净+73.8pp.
- **新生产锚点: 1,001,332/300.53%/1.5137/15.46%** (portfolio.py默认
  QUANT_ZG_FILTER='2'/QUANT_ZG_PEN=0.20, fp豁免, 信号复用).

软罚机制: effective_score += −0.20 (追高行), 与no_chan_penalty同通道。
后续臂对照基准=新锚点. 探针产物: /tmp/probe_chase_years_20260923.txt,
/tmp/probe_chase_score_20260923.txt.

### 17.14: E-RB1排名混合权重bracket — MDD约束绑定, 生产0.3=约束最优 (9/23 09:35)

QUANT_RANK_WITHIN_W (blended_normal=(1−w)×cross + w×within, 生产0.3从未bracket)
在新锚点(含ZG软罚)上三臂:

| w | NAV | 收益 | Sharpe | MDD | 裁决 |
|---|---|---|---|---|---|
| 0.0 | 941,628 | 276.65 | 1.4557 | 15.47 | 0-4否决 |
| **0.3 (生产)** | **1,001,332** | **300.53** | **1.5137** | **15.46** | 锚点 |
| 0.4 | 1,012,425 | 304.97 | 1.5238 | 15.64 | 3-1 MDD败 |
| 0.5 | 1,148,625 | 359.45 | 1.6538 | 15.86 | 3-1 MDD败 |

**裁决**: NAV/收益/Sharpe随w严格单调上升(0.5处+147k/+59pp/+0.14),
但MDD亦单调上升(15.46→15.64→15.86, ~0.02pp/0.01w), 无内点可过铁律。
生产0.3 = **MDD约束下的最优解** (约束恰好绑定, 非自由峰)。家族关闭,
w=0.3保持。年分解佐证: w=0.5的MDD恶化集中在2025(15.9 vs 15.5)。
教训: 单调上升的收益面被单一约束截断时, 内点bracket(0.4)是必要的证伪步。

### 17.15: no_chan_penalty bracket前置探针 — 零行为关闭 (9/23 09:45)

probe_nochan_20260923 (zero-write, 流式): 无结构buy行594,123 (50.2%),
信号层mean +0.42%近零EV (2022/2023微负, 其余正); **无结构入场行n=75,
mean +2.07% 正** (2022 +2.61/2025 +3.23/2026 +4.96, 仅2024 −1.67)。
入场score分位 p25/p50/p75 = 0.547/0.770/0.886 vs 全无结构行p50 0.208
→ **−0.20惩罚已是自洽过滤器**: 低分无结构junk被挡(全层近零EV),
高分无结构赢家照常入场(+2.07%)。−0.30=切赢家, −0.10=放junk,
两方向皆负期望 → bracket臂零理由, 关闭. #207完成。

### 17.16: F-A中性化机制探针 — 聚合惰性闭合 (9/23 09:55)

probe_neutralize_fa (raw缓存5868d425, 806日×~65非噪声因子配对rank-IC):
**非噪声因子聚合 ic_raw=−0.00044 → ic_dem=+0.00103, |ΔIC|=0.0015<0.005,
改善占比=0.50(硬币翻转), t均值=+0.37** → 行业剥离在聚合层零效应。
个别因子强响应(limit_pullback_score t=+11.45/−0.069→−0.027,
...
### 17.18: F-C池过滤器家族 — 池级绑定但入场级方向有害, 关闭 (9/23 10:40)

两探针双层证据 (probe_pool_filters_fc + probe_fc_entry_bands, #215):
**池级绑定度** (403,057 member-boundary对, 93边界): min_price 2→5.0 出17.53%
/2→3.0 出4.29%; relax_momentum None→0.1 出49.99%→0.2 出55.11%; relax
1.0→0.05 净进134.93% (松弛路径=池的定义, 各边界49-80%成员走relax);
**60bar截断cut宽度0.1-1.2% = 非绑定关闭**; 流动fail桶≤0.4%=真拒绝极少。
**入场级方向** (568入场行×qfq实际价/量): 价格带 [2,3) n=9 +3.08% /
[3,5) n=25 +4.05% — 低价入场**优于**主池(≥5元 +2.41%), min_price上调
=砍正EV行 → F-C1关闭; mom20<10%占入场78%全正(mom<0 +2.76%/[0,10%)
+2.07%)且amt<1入场46%为正最高段([0.05,0.2) n=30 **+5.15%** win76.7%,
2021 +12.40% = 全带最高) → relax_momentum门与relax_floor收紧均砍
**最佳表现段**(反转/微盘lottery腿=松弛路径故意放行且付费) → F-C2/F-C3关闭;
min_price 2→1.0方向(+1.34%近退市股)方向自明关闭。
