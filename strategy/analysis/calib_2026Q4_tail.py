...
def main():
    ...
    # 阈值=窗口1193日的10%。9/22干跑实证: 60阈值让4/2026年份概念(gated 101-119日,
    # 份额8.5-9.7%)拿到fresh重标定, 产物不稳定 — 大盘价值/行业龙头坍缩为
    # volatility单因子, 一季报预增fund_pg_improve整因子被弃 = 薄样本噪声签名;
    # 120把它们纳入Q3先验。9/30投影: 中报4≈56日, 一季报预增≈116日, 电池技术
    # ≈128日 → 120把边界放在10%份额处(4.7-9.7%回退, 10.7%重标定), 相同行为带
    # =[120,~128]; 60/100会放一季报档去噪声重标定。9/30写盘后验证器复核。
    GATED_DAY_MIN = 120
